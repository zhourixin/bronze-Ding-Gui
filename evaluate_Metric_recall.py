...
def EVAL_recall(pth_path, saved_excel_path):
    ...
    if eval_model == 1:
        ...
        record_list = []
        for i in OA_Per_Class:
            record_list.append(round(100*float(i),2))
        ...
        classes_number = 2
        OA_labels = torch.load(os.path.join(pth_path, "label0.pth")) #read cpu label Tensor 
        model_predict = torch.load(os.path.join(pth_path, "predict0.pth"))#read cpu model output Tensor 
