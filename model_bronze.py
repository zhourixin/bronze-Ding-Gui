...
class AKG(nn.Module):
    ...
    def forward(self, x, get_feature=False):
        ...
        y_order_sig = self.classifier_1(self.relu(x_cat_fc + x_order_fc+x_species_fc.detach().clone()))

        y_species_sig = self.classifier_2(self.relu(x_cat_fc + x_order_fc + x_species_fc))
        y_species_sof = self.classifier_2_1(self.relu(x_cat_fc + x_order_fc + x_species_fc))

        # 修改后
        # y_cat_sig = self.classifier_0(self.relu(x_cat_fc + x_order_fc.detach().clone() + x_species_fc.detach().clone()))

        # y_order_sig = self.classifier_1(self.relu(x_order_fc+x_species_fc.detach().clone()))

        # y_species_sig = self.classifier_2(self.relu(x_order_fc + x_species_fc))
        # y_species_sof = self.classifier_2_1(self.relu(x_order_fc + x_species_fc))
        ...
        if get_feature:
            return x_cat_fc + x_order_fc + x_species_fc


        return y_cat_sig, y_order_sig, y_species_sof, y_species_sig, shape_sof, shape_sig, y_att_sig
