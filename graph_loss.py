...
class GraphLoss(nn.Module):
    def __init__(self, hierarchy, total_nodes, levels, device, args):
        ...
        self.ShapeStateSpace = self.generateShapeStateSpace(total_nodes, self.stateSpace, shape_nodes=35, age_intersection=True).to(device)      
        self.AttStateSpace = self.generateAttStateSpace(total_nodes, self.stateSpace, att_nodes=149, age_intersection=True).to(device)

        self.alph1 = args.alph1
        self.alph2 = args.alph2
        

    def forward(self, fs, labels, device, label_fs,labels_shape, attribute_label, att_fs, cat_label): 
        ...
        for i in range(len(labels)):
            ...
            if labels[i] > 999:
                # print(1)
                continue
            # a = torch.where(self.stateSpace[:, labels[i]] > 0)
            b = torch.where(self.stateSpace[:, labels[i]] > 0)[0]
            # c = torch.where(self.stateSpace[:, int(cat_label[i])] > 0 )
            d = torch.where(self.stateSpace[:, int(cat_label[i])] > 0 )[0]
            tensor_isin = torch.isin(b,d)
            b = b[tensor_isin]
            marginal = torch.sum(torch.index_select(joint[:, i], 0, b))
            p = marginal / z[i]
            p = torch.clamp(p, thresh, 1-thresh)
            p_array[i] = p
            loss[i] = -torch.log(p)
...
################################################################################
        ...
        for i in range(len(labels_shape)):
            ...
            pt = torch.clamp(pt, thresh, 1-thresh)
            p_array_shape[i] = pt
    ...
    def generateStateSpace(self, hierarchy, total_nodes, levels):
        ...
        if levels == 2:
            ...
            i = 1
            recorded = torch.zeros(2, total_nodes)
            ...
            stateSpace = torch.zeros((total_nodes-1)*2 + 1, total_nodes)
            i = 1
            recorded = torch.zeros(2, total_nodes)
            for path in hierarchy:
                if recorded[path[2], path[2]] == 0:
                    stateSpace[i, path[2]] = 1
                    recorded[path[2], path[2]] = 1
                    i += 1
                if recorded[path[2], path[1]] == 0:
                    stateSpace[i, path[2]] = 1
                    stateSpace[i, path[1]] = 1
                    recorded[path[2], path[1]] = 1
                    i += 1
                stateSpace[i, path[1]] = 1
                stateSpace[i, path[2]] = 1
                stateSpace[i, path[0]] = 1
            ...
            #     print(j)
            #     print(stateSpace[j])
            if i == (total_nodes-1)*2 + 1:
                return stateSpace
            else:
                print('Invalid StateSpace!!!')

    def generateShapeStateSpace(self, total_nodes, origin_stateSpace, shape_nodes=29, age_intersection=False):
        ...
        if age_intersection:
            ...
            shape_index = 0
            for state_index in range(1,(total_nodes-1)*2+1):
            ...
            shape_index = 0
            for state_index in range(0,total_nodes+1):
                legal_state = origin_stateSpace[state_index, :].unsqueeze(0)
                for i in range(shape_nodes):
                    shape_state = torch.zeros((1,shape_nodes))
                    shape_state[0][i] = 1
    ...
    def generateAttStateSpace(self, total_nodes, origin_stateSpace, att_nodes=96, age_intersection=False):
