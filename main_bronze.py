...
if __name__ == '__main__':
    ...
    for EXP_number in range(1,5):
        ...
        train_excel_path = DATASET_ROOT+"/excel_nc/fold{}_train.xlsx".format(EXP_number)
        val_excel_path = DATASET_ROOT+"/excel_nc/fold{}_test.xlsx".format(EXP_number)
        test_excel_path = DATASET_ROOT+"/excel_nc/fold{}_test.xlsx".format(EXP_number)
        ...
        if args.lr_adjt == "Step":
            optimizer = torch.optim.SGD(net.parameters(), lr = args.lr, momentum = 0.9, weight_decay=5e-4)
            scheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size = 50, gamma = 0.1)
        elif args.lr_adjt == "Cos":
        ...
        """evaluate"""
        npy_path = os.path.join(exp_ROOT, "Metric_data", "fold{}".format(EXP_number))
        if os.path.exists(npy_path) is False:
            os.mkdir(npy_path)
        record_file = os.path.join(npy_path, "final_record.csv")
